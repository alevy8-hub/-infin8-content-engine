...
const saved = JSON.parse(localStorage.getItem('infin8-content-engine') || '{}');
...
function keywords(text){
  const stop = new Set('the a an and or but if then to of in on for with is are was were be been being it this that these those i you we they he she them our your my from as at by about into than so just'.split(' '));
  const words = cleanText(text).toLowerCase().match(/[a-z0-9$#]+/g) || [];
  const counts = {};
  words.forEach(w => { if(!stop.has(w) && w.length > 3) counts[w]=(counts[w]||0)+1; });
  return Object.entries(counts).sort((a,b)=>b[1]-a[1]).slice(0,4).map(([w])=>w);
}
...
function transform(kind, text){
  ...
  const tags = keys.length ? keys.map(k => `#${k.replace(/[^a-z0-9]/g,'')}`).join(' ') : '#content #creator';

  switch(kind){
    case 'x':
      return `${opener}\n\n${two !== opener ? two : ''}\n\nCreate once. Distribute iNFiN8. ♾️`.trim().slice(0, 900);
    case 'linkedin':
...
function renderOutputs(text){
  ...
  platforms.forEach((platform, index) => {
    ...
    const card = node.querySelector('.asset');
    ...
    const previous = saved.outputs && saved.outputs[platform.kind];
    output.value = previous || transform(platform.kind, text);
    ...
    output.addEventListener('input', saveState);
